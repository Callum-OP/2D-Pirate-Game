...
public class UIScript : MonoBehaviour
{
	...
	public void AddPoints(int playerNumber, int amount = 1)
	{
        ...
        GlobalScore.PlayerScore = scores[playerNumber];
        ...
        if (scores[playerNumber] >= 500)
        {
            GameObject wallToDestroy = GameObject.Find("Exit Door 1");
            Destroy(wallToDestroy);
        }
        if (scores[playerNumber] >= 1000)
        {
            GameObject wallToDestroy = GameObject.Find("Exit Door 2");
            Destroy(wallToDestroy);
        }
        if (scores[playerNumber] >= 1500)
            {
                GameObject wallToDestroy = GameObject.Find("Exit Door 3");
                Destroy(wallToDestroy);
            }
